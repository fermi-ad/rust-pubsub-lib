...
## Interface 
The primary abstractions provided by this library are the `Publisher` and `Subscriber` structs. They expose a predefined set of methods for publishing and subscribing to messages on a given topic.

#### Required environment variables
For this lib to operate successfully, the following environment variables must be set:
- `KAFKA_HOST_ADDR` -> At time of writing, Kafka is the message broker/pub-sub service of choice. This variable specifies the location of the Kafka instance to connect to.
